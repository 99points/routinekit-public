...
class Stepwise_SaaS_Auth {
	...
	public static function connect( string $license_key ) {
		...
		if ( empty( $result['api_key'] ) ) {
			return new WP_Error( 'stepwise_saas_error', __( 'Invalid response from SaaS.', 'stepwise' ) );
		}

		update_option( 'stepwise_site_api_key',        sanitize_text_field( $result['api_key'] ) );
		update_option( 'stepwise_site_id',             sanitize_text_field( (string) ( $result['site_id'] ?? '' ) ) );
		update_option( 'stepwise_saas_team',           sanitize_text_field( $result['team_name'] ?? '' ) );
		update_option( 'stepwise_license_key',         sanitize_text_field( $license_key ) );
		// Store the URL this site connected with so clones can be detected locally before the heartbeat fires.
		update_option( 'stepwise_registered_site_url', get_site_url() );
		...
		foreach ( [ 'stepwise_license_plan', 'stepwise_site_api_key', 'stepwise_site_id', 'stepwise_saas_team', 'stepwise_license_key' ] as $key ) {
		...
		if ( ! wp_next_scheduled( 'stepwise_saas_heartbeat' ) ) {
			wp_schedule_event( time(), 'hourly', 'stepwise_saas_heartbeat' );
		}

		// Flush full page cache so the admin page HTML is regenerated with updated stepwiseData.
		wp_cache_flush();
		do_action( 'stepwise_cache_flush' );
	...
	public static function disconnect(): void {
		...
		delete_option( 'stepwise_site_id' );
		delete_option( 'stepwise_saas_team' );
		delete_option( 'stepwise_license_key' );
		delete_option( 'stepwise_license_plan' );
		delete_option( 'stepwise_last_sync' );
		delete_option( 'stepwise_registered_site_url' );
	...
	public static function is_connected(): bool {
		return ! empty( get_option( 'stepwise_site_api_key' ) );
	}
}
