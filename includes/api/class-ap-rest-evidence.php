 ...
 * POST /alignpress/v1/executions/:execution_id/steps/:step_id/evidence
 *
...
class AP_REST_Evidence {
	...
	public function upload_evidence( WP_REST_Request $request ): WP_REST_Response|WP_Error {
		...
		if ( ! $completion ) {
			return new WP_Error(
				'alignpress_not_found',
				__( 'Step completion not found.', 'alignpress' ),
				[ 'status' => 404 ]
			);
		}
		...
		if ( empty( $files['evidence'] ) ) {
			return new WP_Error(
				'alignpress_missing_file',
				__( 'No file uploaded. Send the file as multipart/form-data with key "evidence".', 'alignpress' ),
				[ 'status' => 400 ]
			);
		}
		...
		if ( ! $check['type'] || ! in_array( $check['type'], $allowed_types, true ) ) {
			return new WP_Error(
				'alignpress_invalid_file',
				__( 'Unsupported file type. Allowed: JPEG, PNG, GIF, WebP, PDF.', 'alignpress' ),
				[ 'status' => 415 ]
			);
		}
		...
		if ( $files['evidence']['size'] > 10 * MB_IN_BYTES ) {
			return new WP_Error(
				'alignpress_file_too_large',
				__( 'Evidence file must be 10 MB or smaller.', 'alignpress' ),
				[ 'status' => 413 ]
			);
		}
	...
	public function permissions_check( WP_REST_Request $request ): bool|WP_Error {
		...
		return new WP_Error(
			...
			__( 'You do not have permission to upload evidence.', 'alignpress' ),
			[ 'status' => 403 ]
		);
	}
	...
	private function get_completion( int $execution_id, int $step_id ): ?object {
		...
		if ( current_user_can( 'manage_options' ) ) {
			return $wpdb->get_row(
				$wpdb->prepare(
					"SELECT * FROM {$wpdb->prefix}alignpress_step_completions
		...
		return $wpdb->get_row(
			$wpdb->prepare(
				"SELECT sc.* FROM {$wpdb->prefix}alignpress_step_completions sc
				 INNER JOIN {$wpdb->prefix}alignpress_executions e ON e.id = sc.execution_id
