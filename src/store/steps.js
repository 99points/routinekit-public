...
const actions = {
	...
	fetchSteps: ( workflowId ) => async ( { dispatch } ) => {
		...
		try {
			const steps = await apiFetch( {
				path: `/routinekit/v1/workflows/${ workflowId }/steps`,
			} );
	...
	saveStep: ( workflowId, stepId, data ) => async ( { dispatch } ) => {
		...
		try {
			const step = await apiFetch( {
				path: `/routinekit/v1/workflows/${ workflowId }/steps/${ stepId }`,
				method: 'PATCH',
				data,
			} );
	...
	reorderSteps: ( workflowId, order ) => async ( { dispatch } ) => {
		try {
			const steps = await apiFetch( {
				path: `/routinekit/v1/workflows/${ workflowId }/steps/reorder`,
				method: 'PATCH',
				data: { order },
			} );
			dispatch( actions.setSteps( workflowId, steps ) );
...
const store = createReduxStore( 'routinekit/steps', { reducer, actions, selectors } );
register( store );
